# Learn Concepts Feature — Full Code
**Files:**
- `src/features/learn/data/conceptsData.ts`
- `src/features/learn/hooks/useConcepts.ts`
- `app/(tabs)/learn.tsx`
- `app/concept/[id].tsx`

> The "Learn Concepts" tab teaches foundational CS theory: Big-O notation, data structures, common patterns, interview tips — organized as cards with rich detail views.
...
export const CONCEPT_CATEGORIES: ConceptCategory[] = [
  'COMPLEXITY', 'DATA_STRUCTURES', 'ALGORITHM_PATTERNS', 'INTERVIEW_TIPS', 'MATH_FOUNDATIONS',
];
...
import { CONCEPTS, Concept, ConceptCategory } from '../data/conceptsData';
...
import { Concept } from '../../src/features/learn/data/conceptsData';
import { CATEGORY_META, CONCEPT_CATEGORIES } from '../../src/features/learn/data/conceptsData';
  ...
  return (
    <SafeAreaView style={styles.screen}>
      ...
      <View style={styles.searchContainer}>
        <Ionicons name="search" size={18} color="#6B6B8A" style={styles.searchIcon} />
        <TextInput style={styles.searchInput} value={searchQuery} onChangeText={setSearchQuery} placeholder="Search concepts..." placeholderTextColor="#6B6B8A" />
      </View>
      ...
      <FlatList
        ...
        renderItem={({ item: cat }) => {
          const meta = cat === 'ALL' ? { label: 'All', color: '#6C63FF' } : CATEGORY_META[cat as any];
          const isSelected = selectedCategory === cat;
          return (
            <TouchableOpacity onPress={() => setSelectedCategory(cat as any)}
              style={[styles.filterChip, isSelected && { backgroundColor: meta.color }]}>
              <Text style={[styles.filterChipText, isSelected && styles.filterChipTextActive]}>{cat === 'ALL' ? 'All' : meta.label}</Text>
            </TouchableOpacity>
          );
        }}
      />
      ...
      <FlatList
        ...
        ListEmptyComponent={<Text style={styles.emptyText}>No concepts found</Text>}
      />
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  searchContainer: { flexDirection: 'row', alignItems: 'center', marginHorizontal: 16, marginBottom: 8, backgroundColor: '#1E1E2E', borderRadius: 12, paddingHorizontal: 12, gap: 8 },
  searchIcon: { },
  ...
  filterChipText: { color: '#9E9EB8', fontSize: 13 },
  ...
  emptyText: { textAlign: 'center', color: '#6B6B8A', marginTop: 40 },
});
```
